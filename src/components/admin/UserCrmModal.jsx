...
const UserCrmModal = ({ user, onClose, onSubscriptionAction }) => {
  ...
  return (
    <div className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center p-4" onClick={onClose}>
      <div
        className="bg-zinc-900 border border-zinc-700 rounded-2xl w-full max-w-lg max-h-[85vh] overflow-y-auto p-4 md:p-6"
      ...
      >
        ...
        {loading ? (
          ...
          <div className="space-y-6">
            ...
            {/* Payment history */}
            <div>
              <label className="text-xs uppercase font-bold text-zinc-500 block mb-2">היסטוריית תשלומים</label>

              {sortedPayments.length === 0 ? (
                <p className="text-zinc-500 text-sm mb-3">אין תשלומים רשומים.</p>
              ) : (
                  ...
                  {sortedPayments.map((p) => (
                    <div key={p.id} className="flex items-center justify-between bg-black/40 border border-zinc-800 rounded-xl p-2.5">
                      <div className="text-sm">
                        ...
                        <span className="text-zinc-400">{p.method}</span>
                        {p.note && <span className="text-zinc-500 mx-2">— {p.note}</span>}
                      </div>
                      <button onClick={() => handleDeletePayment(p.id)} className="text-red-500 hover:text-red-400">
                        <Trash2 size={16} />
                      </button>
                    </div>
                  ))}
