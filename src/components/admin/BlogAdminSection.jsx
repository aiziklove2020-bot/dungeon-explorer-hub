                ...
                {editingCommentId === comment.id ? (
                  ...
                  <>
                    <p className="text-zinc-500 text-xs mb-2">
                      <span className="text-zinc-300 font-bold">{comment.authorName}</span> (ID: {comment.authorId})
                    ...
                    <div className="flex gap-2 mt-3">
                      <button onClick={() => startEditComment(comment)} className="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1"><Pencil size={12} /> ערוך</button>
                      <button onClick={() => handleDeleteComment(comment.id)} className="bg-red-900 hover:bg-red-800 text-white px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1"><Trash2 size={12} /> מחק</button>
                    </div>
                  </>
                )}
              </div>
  ...
  return (
    <div className="bg-[#121218] backdrop-blur-2xl border border-white/5 p-4 md:p-6 rounded-xl md:rounded-2xl space-y-4 md:space-y-6">
      ...
      {loading ? <AdminLoader /> : posts.length === 0 ? (
        <p className="text-zinc-500 text-sm text-center py-8">אין פוסטים עדיין</p>
      ) : (
        <div className="space-y-3">
          {posts.map(post => (
              ...
              {editingPostId === post.id ? (
                ...
                <>
                  <div className="flex justify-between items-start gap-3">
                    ...
                    <button onClick={() => openPost(post)} className="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1 shrink-0">
                      <MessageSquare size={12} /> תגובות
                    </button>
                  </div>
                  <div className="flex gap-2 mt-3">
                    <button onClick={() => startEditPost(post)} className="bg-zinc-800 hover:bg-zinc-700 text-white px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1"><Pencil size={12} /> ערוך</button>
                    <button onClick={() => handleDeletePost(post.id)} className="bg-red-900 hover:bg-red-800 text-white px-3 py-1 rounded-lg text-xs font-bold flex items-center gap-1"><Trash2 size={12} /> מחק</button>
                  </div>
                </>
              )}
            </div>
